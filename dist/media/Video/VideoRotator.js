...
export const VideoRotator = ({ videos, preloadFirstVideo = true, // Default to true
 }) => {
    const [currentIndex, setCurrentIndex] = useState(0);
    const videoRef = useRef(null);
    const preloadedVideosRef = useRef(new Map());
    // Calculate the total preloaded video duration
    const getTotalPreloadedDuration = useCallback(() => {
        let total = 0;
    ...
    const preloadNext = useCallback((indexToPreload) => {
        if (indexToPreload >= videos.length)
        ...
        if (!videoData)
            return; // Should not happen with the length check, but good practice
    ...
    if (!videos || videos.length === 0) {
        return _jsx("div", { children: "No videos to display." }); // Handle empty or invalid videos array
    }
    const currentVideo = videos[currentIndex];
    return (_jsx("div", { className: "VideoRotator relative w-full h-full", children: _jsx(Video, { ref: videoRef, videoSources: { webm: currentVideo.webm, mp4: currentVideo.mp4 }, poster: currentVideo.poster, onEnded: handleVideoEnded, 
            // Preload the first video based on prop, preload subsequent videos immediately when they become current
            preloadImmediately: currentIndex === 0 ? preloadFirstVideo : true }, currentIndex) }));
};
// Add display name for better debugging
VideoRotator.displayName = 'VideoRotator';
//# sourceMappingURL=VideoRotator.js.map